...
import com.vn.security.core.security.store.SecMenuDefinitionStore;
import com.vn.security.core.security.store.SecMenuPermissionStore;
import com.vn.security.core.service.dto.security.SecMenuDefinitionDTO;
import com.vn.security.core.service.dto.security.SyncNodeDTO;
import com.vn.security.core.service.dto.security.SyncResultDTO;
...
public class SecMenuDefinitionAdminResource {
    ...
    private final SecMenuDefinitionStore secMenuDefinitionStore;

    private final SecMenuPermissionStore secMenuPermissionStore;

    public SecMenuDefinitionAdminResource(
        SecMenuDefinitionStore secMenuDefinitionStore,
        SecMenuPermissionStore secMenuPermissionStore
    ) {
        this.secMenuDefinitionStore = secMenuDefinitionStore;
    ...
    public ResponseEntity<SecMenuDefinitionDTO> createMenuDefinition(@Valid @RequestBody SecMenuDefinitionDTO dto)
        ...
        if (secMenuDefinitionStore.findByAppNameAndMenuId(dto.getAppName(), dto.getMenuId()).isPresent()) {
        ...
        entity = secMenuDefinitionStore.save(entity);
    ...
    public ResponseEntity<List<SecMenuDefinitionDTO>> getAllMenuDefinitions(@RequestParam(required = false) String appName) {
        ...
        List<SecMenuDefinitionDTO> dtos = (
            appName == null || appName.isBlank()
                ? secMenuDefinitionStore.findAllByOrderByAppNameAscOrderingAscIdAsc()
                : secMenuDefinitionStore.findAllByAppNameOrderByOrderingAscIdAsc(appName)
        ).stream()
            .map(this::toDto)
            .toList();
    ...
    public ResponseEntity<SecMenuDefinitionDTO> getMenuDefinition(@PathVariable("id") Long id) {
        ...
        return ResponseUtil.wrapOrNotFound(secMenuDefinitionStore.findById(id).map(this::toDto));
    }
    ...
    public ResponseEntity<SecMenuDefinitionDTO> updateMenuDefinition(
        ...
        SecMenuDefinition existing = secMenuDefinitionStore
            .findById(id)
        ...
        existing = secMenuDefinitionStore.save(existing);
    ...
    public ResponseEntity<Void> deleteMenuDefinition(@PathVariable("id") Long id) {
        ...
        SecMenuDefinition entity = secMenuDefinitionStore
            .findById(id)
            .orElseThrow(() -> new BadRequestAlertException("Menu definition not found", ENTITY_NAME, "notfound"));
        MenuAppName.fromValue(entity.getAppName()).ifPresent(appName ->
            secMenuPermissionStore.deleteByAppNameAndMenuId(appName, entity.getMenuId())
        );
        secMenuDefinitionStore.deleteById(id);
    ...
    public ResponseEntity<SyncResultDTO> syncFromRegistry(@Valid @RequestBody List<SyncNodeDTO> nodes) {
        ...
        for (SyncNodeDTO node : nodes) {
            if (secMenuDefinitionStore.findByAppNameAndMenuId(node.getAppName(), node.getMenuId()).isPresent()) {
                skipped++;
            } else {
                ...
                secMenuDefinitionStore.save(entity);
                seeded++;
            }
        }
