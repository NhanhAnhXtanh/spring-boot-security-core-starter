...
import com.vn.security.core.security.access.AccessManager;
import com.vn.security.core.security.access.CrudEntityContext;
...
import com.vn.security.core.security.merge.SecureMergeService;
import com.vn.security.core.security.permission.EntityOp;
import com.vn.security.core.security.serialize.SecureEntitySerializer;
...
public class SecureDataManagerImpl implements SecureDataManager {
    ...
    private final SecureJpqlQueryExecutor secureJpqlQueryExecutor;
    ...
    public SecureDataManagerImpl(
        ...
        SecureJpqlQueryExecutor secureJpqlQueryExecutor
    ) {
        ...
        this.secureJpqlQueryExecutor = secureJpqlQueryExecutor;
    }
    ...
    private <E> Page<E> loadListInternal(Class<E> entityClass, Pageable pageable) {
        ...
        return unconstrainedDataManager.loadPage(entityClass, null, pageable);
    }
    ...
    private <E> Page<E> loadByQueryInternal(Class<E> entityClass, SecuredLoadQuery query, SecuredEntityEntry entry) {
        if (query.jpql() != null && !query.jpql().isBlank()) {
            ...
            checkCrud(entityClass, EntityOp.READ);
            return secureJpqlQueryExecutor.execute(entityClass, query);
        }

        checkCrud(entityClass, EntityOp.READ);
        Specification<E> filterSpec = secureQuerySpecificationFactory.build(entityClass, query.parameters());
        return unconstrainedDataManager.loadPage(entityClass, filterSpec, query.pageable());
    }
    ...
    private <E> Optional<E> loadOneInternal(Class<E> entityClass, Object id) {
        ...
        checkCrud(entityClass, EntityOp.READ);
        E entity = unconstrainedDataManager.load(entityClass, id);
    ...
    private <E> E saveInternal(Class<E> entityClass, Object id, EntityMutation<E> mutation) {
        ...
        E entity;
        if (id == null) {
            checkCrud(entityClass, EntityOp.CREATE);
            ...
            entity = unconstrainedDataManager
                .loadOne(entityClass, idSpec)
    ...
    private <E> void deleteInternal(Class<E> entityClass, Object id) {
        ...
        E entity = unconstrainedDataManager
            .loadOne(entityClass, idSpec)
