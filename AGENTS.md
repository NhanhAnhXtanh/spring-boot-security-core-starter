...
3. **Persistence layer:** `EntityManager` (qua `SecureDataManager`) cho mọi entity nghiệp vụ. `JpaRepository` chỉ có `UserRepository` + `AuthorityRepository` của starter — **consumer không tạo `JpaRepository` mới**.
4. **Entity mới:** bắt buộc `@SecuredEntity` + `@EntityScan` package consumer + seed permission + fetch plan. Superclass **tuỳ chọn** — nếu consumer đã có `BaseEntity` riêng thì giữ, KHÔNG ép sang `AbstractAuditingEntity`. Xem [`rules/entity-onboarding.md`](rules/entity-onboarding.md).
5. **Khi nghi ngờ** → chọn `SecureDataManager`. Bypass là lỗi mặc định, không phải shortcut.
